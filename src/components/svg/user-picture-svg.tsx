export type UserPictureSVGProps = {
  size?: number;
  fill?: string;
  stroke?: string;
};

export default function UserPictureSVG({ size = 270, fill = '#5B5E5F', stroke = 'white' }: UserPictureSVGProps): JSX.Element {
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width={size} height={size} rx="40" fill={fill} />
      <path
